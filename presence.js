(()=>{
  ...
  const meta={timezone:Intl.DateTimeFormat().resolvedOptions().timeZone||'',language:navigator.language||'',device:detectDevice(),browser:detectBrowser(),referrer_host:referrerHost()};
  ...
  const addGui=()=>{
    ...
    const style=document.createElement('style');style.id='dm-live-visitors-style';style.textContent=`
      ...
      .dm-live-pill{display:flex;align-items:center;gap:8px;border:1px solid rgba(255,255,255,.11);background:linear-gradient(180deg,rgba(19,20,24,.97),rgba(11,12,15,.97));color:#f5f5f7;border-radius:13px;padding:10px 13px;box-shadow:0 14px 36px rgba(0,0,0,.38),0 0 20px rgba(220,35,61,.08);backdrop-filter:blur(14px);cursor:pointer;font:800 12px/1 system-ui,-apple-system,"Segoe UI",sans-serif;transition:transform .18s ease,border-color .18s ease,box-shadow .18s ease}
      .dm-live-pill:hover{transform:translateY(-2px);border-color:rgba(239,79,95,.36);box-shadow:0 18px 42px rgba(0,0,0,.44),0 0 24px rgba(220,35,61,.13)}
      .dm-live-dot{width:7px;height:7px;border-radius:50%;background:#55dc86;box-shadow:0 0 9px rgba(85,220,134,.7);flex:0 0 auto}.dm-live-count{font-size:12px;color:#fff;min-width:10px;text-align:center}.dm-live-label{font-size:10px;color:#8d9199;font-weight:750}
      .dm-live-panel{position:absolute;right:74px;bottom:0;width:320px;max-height:min(560px,70vh);overflow:auto;padding:14px;border:1px solid rgba(255,255,255,.11);border-radius:16px;background:linear-gradient(180deg,rgba(14,15,18,.985),rgba(8,9,11,.985));box-shadow:0 24px 64px rgba(0,0,0,.55),0 0 30px rgba(197,18,47,.07);backdrop-filter:blur(18px);opacity:0;transform:translateY(8px) scale(.98);pointer-events:none;transition:opacity .18s ease,transform .18s ease}
      #dmLiveVisitors.open .dm-live-panel{opacity:1;transform:none;pointer-events:auto}
      .dm-live-head{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:2px 2px 10px;border-bottom:1px solid rgba(255,255,255,.08)}.dm-live-head b{font-size:12px}.dm-live-head span{font-size:9px;color:#ef5368;font-weight:900;letter-spacing:.1em;text-transform:uppercase}
      ...
      .dm-live-section-title{display:flex;justify-content:space-between;align-items:center;padding:7px 2px 8px;color:#9b9ba5;font-size:9px;font-weight:900;text-transform:uppercase;letter-spacing:.08em;border-top:1px solid rgba(255,255,255,.07)}
      .dm-visitor{padding:9px 8px;border:1px solid rgba(255,255,255,.06);border-radius:11px;background:rgba(255,255,255,.022);margin-bottom:7px}.dm-visitor-top{display:flex;align-items:center;justify-content:space-between;gap:10px}.dm-visitor-name{font-size:11px;font-weight:900}.dm-visitor-place{font-size:10px;color:#6fdfa0}.dm-visitor-meta,.dm-visitor-path{font-size:9px;color:#85858f;margin-top:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.dm-visitor-path{color:#b8b8c1}.dm-live-empty{padding:10px 4px;color:#777782;font-size:10px}.dm-live-updated{padding:9px 2px 0;color:#666670;font-size:9px;text-align:right}
      @media(max-width:760px){#dmLiveVisitors{right:16px;bottom:158px}.dm-live-panel{right:0;bottom:54px;width:min(320px,calc(100vw - 24px));max-height:calc(100vh - 250px)}.dm-live-pill{padding:9px 11px}}
  ...
  const statsTimer=setInterval(()=>{if(document.visibilityState==='visible'&&ownerKey())loadStats()},10000);
  document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'){ping();if(ownerKey())loadStats()}});
